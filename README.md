...
| <a name="input_gke_min_version"></a> [gke\_min\_version](#input\_gke\_min\_version) | The GKE version for the cluster. If not set, the default version for the selected release channel will be used. | `string` | `""` | no |
| <a name="input_gke_node_disk_size_gb"></a> [gke\_node\_disk\_size\_gb](#input\_gke\_node\_disk\_size\_gb) | Size of the node root volume in GB. | `number` | `"100"` | no |
| <a name="input_gke_release_channel"></a> [gke\_release\_channel](#input\_gke\_release\_channel) | The release channel for the GKE cluster. Can be one of `RAPID`, `REGULAR`, or `STABLE`. | `string` | `"STABLE"` | no |
...
| <a name="input_operator_chart_version"></a> [operator\_chart\_version](#input\_operator\_chart\_version) | Version of the operator chart to deploy | `string` | `"1.3.12"` | no |
